...
| README.md | HTTPDNS Android客户端接入文档 | 接入文档 |
| SDK接入是否成功验证方法.docx | HTTPDNS Android SDK接入验证 | SDK开发人员维护 |
| VERSION.md | HTTPDNS Android SDK历史版本修改记录 | SDK开发人员维护 |
...
### 2.4 HttpDns Java接口调用：
	...
	* @param String openId
	*/
	WGGetHostByName.getInstance().WGSetDnsOpenId("NULL");
	...
	*/
	String ips = WGGetHostByName.getInstance(). getAddrByName(domain);
...
### 3.2 如果客户端的业务是与host绑定的，比如是绑定了host的http服务或者是cdn的服务，那么在用HTTPDNS返回的IP替换掉URL中的域名以后，还需要指定下Http头的Host字段。
	- 以URLConnection为例： 
        ...
        // 获取HttpDns域名解析结果 
        String ips = WGGetHostByName.getInstance().getAddrByName(oldUrl.getHost()); 
...
### 注意：若已接入msdk或者单独接入了腾讯灯塔则不用初始化灯塔。
	...
	public static void Init() {
		...
		// 初始化HttpDns
		AndroidJavaObject joDnsClass = new AndroidJavaObject("com.tencent.msdk.dns.WGGetHostByName");
...
## 3. OkHttp+HttpDns场景
    ...
    String ip = WGGetHostByName.getInstance().getAddrByName("www.qq.com"); // HttpDns解析域名
    if (ip != null) {
	    if (ip.contains(";")) {
